// Stealth cross-language oracle — Dart driver. Mirrors run-oracle.mjs.
import 'stealth.dart';

const DT = 1.0 / 64.0;
int step = 0;
late Stealth g;
final FAR = Vector2(500, 500);
Vector2 pos1 = Vector2(500, 500), pos2 = Vector2(500, 500), pos3 = Vector2(500, 500);

Vector2 P(double x, double y) => Vector2(x, y);
String pad(String s, int w) { var o = s; while (o.length < w) { o += ' '; } return o; }
String flags(Guard gd) => '${gd.is_aware() ? 1 : 0}${gd.is_alerted() ? 1 : 0}${gd.should_move() ? 1 : 0}';
String gcol(Guard gd) {
  final t = gd.get_target();
  return '${gd.get_state()}/${flags(gd)} tgt=(${t.x.round()},${t.y.round()})';
}
String lpad(Object o, int w) { var s = '$o'; while (s.length < w) { s = ' $s'; } return s; }
void snapOf(Stealth m, String label, String tag) {
  final tks = (m.get_elapsed() * 64).round();
  print('$tag${step.toString().padLeft(3, '0')} ${pad(label, 38)} st=${pad(m.get_state(), 8)} '
        't=${lpad(tks, 4)} by=${lpad(m.get_caught_by(), 2)} | '
        'g1=${pad(gcol(m.guard1), 28)} | g2=${pad(gcol(m.guard2), 28)} | g3=${pad(gcol(m.guard3), 28)}');
  step++;
}
void snap(String label) => snapOf(g, label, '');
void pump(int n) { for (var i = 0; i < n; i++) { g.tick(DT, pos1, pos2, pos3); } }
void run(int n, String label) { pump(n); snap('pump x$n ($label)'); }

void main() {
  final P1 = [P(0, 0), P(64, 0), P(64, 64)];
  final P2 = [P(0, 0), P(96, 0)];
  final P3 = [P(0, 0), P(96, 96)];
  g = Stealth.create();
  ...
  final esc = Stealth.create();
  esc.start(P1, P2, P3);
  for (var i = 0; i < 64; i++) { esc.tick(DT, FAR, FAR, FAR); }
  esc.player_at_exit();
  print('ESC escape path: st=${esc.get_state()} by=${esc.get_caught_by()} t=${(esc.get_elapsed() * 64).round()}');

  g.start(P2, P3, P1);
  snap('Q: start after restart: init DROPPED');

  step = 0;
  final s = Stealth.create();
  s.start(P1, P2, P3);
  for (var i = 0; i < 32; i++) { s.tick(DT, P(1, 1), FAR, FAR); }
  s.guard1.hear_sound(P(50, 50));
  s.guard2.spot_player(P(80, 80));
  for (var i = 0; i < 32; i++) { s.tick(DT, FAR, FAR, FAR); }
  snapOf(s, 'SAVE POINT (push live, alerted, mid)', 'S');
  final blob = s.save_state();
  final r = Stealth.create();
  r.restore_state(blob);
  snapOf(r, 'restored copy, same tick', 'S');
  step -= 1;
  final plan = [[64, 'invest pops on both'], [224, 'chase times out on both'], [192, 'search resumes patrol on both']];
  for (final pr in plan) {
    final n = pr[0] as int; final label = pr[1] as String;
    for (var i = 0; i < n; i++) { s.tick(DT, FAR, FAR, FAR); }
    for (var i = 0; i < n; i++) { r.tick(DT, FAR, FAR, FAR); }
    snapOf(s, 'orig  +$n ($label)', 'S');
    step -= 1;
    snapOf(r, 'rest  +$n ($label)', 'S');
  }
  s.pause();
  final blob2 = s.save_state();
  final r2 = Stealth.create();
  r2.restore_state(blob2);
  r2.resume();
  print('SP  paused save -> restore -> resume: st=${r2.get_state()} t=${(r2.get_elapsed() * 64).round()}');
}
