// Shooter cross-language oracle — Java driver. Mirrors run-oracle.mjs
...
public class Driver {
    ...
    static void snap(String label) {
        String e0 = g.enemy_count() > 0 ? g.enemies.get(0).get_state() : "-";
        String e1 = g.enemy_count() > 1 ? g.enemies.get(1).get_state() : "-";
    ...
    public static void main(String[] args) {
        ...
        Shooter g2 = Shooter.__create();
        g2.start();
        for (int w = 0; w < 10; w++) { for (int i = 0; i < 129; i++) { g2.tick(DT); } if (g2.should_spawn_wave()) g2.consume_wave(); }
        for (int i = 0; i < 65; i++) { g2.tick(DT); }
        System.out.println("Q00 quirk: state=" + g2.get_state() + " boss=" + g2.boss.get_state());
        g2.boss_hit(90);
        System.out.println("Q01 quirk: one 90-dmg hit in P1 -> boss=" + g2.boss.get_state() + " hp=" + g2.boss.get_hp() + " (phase two, not dying)");
        g2.boss_hit(1);
        System.out.println("Q02 quirk: 1-dmg hit in P2 at 0hp -> boss=" + g2.boss.get_state() + " hp=" + g2.boss.get_hp() + " (phase three)");
        g2.boss_hit(1);
        System.out.println("Q03 quirk: 1-dmg hit in P3 -> boss=" + g2.boss.get_state() + " (finally dying)");
    }
}
