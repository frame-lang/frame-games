# Shooter cross-language oracle — Ruby driver. Mirrors run-oracle.mjs.
...
def build_wave
  ...
  if $waves_built == 1
    ...
    $g.add_enemy(Enemy._create($waves_built % 3, 1, 0.0, 10))
    $g.add_enemy(Enemy._create(($waves_built + 1) % 3, 1, 0.0, 10))
  end
end
...
def pump(n)
  n.times do
    ...
    $g.consume_boss_spawn if $g.should_spawn_boss
    (0...$g.enemy_count).each do |e|
      en = $g.enemies[e]
      if en.wants_to_fire
        en.consume_fire; $ef += 1
      end
    end
    if $g.boss.wants_to_fire_single then $g.boss.consume_fire; $bsingle += 1 end
    if $g.boss.wants_to_fire_spread then $g.boss.consume_fire; $bspread += 1 end
    if $g.boss.wants_to_fire_spray  then $g.boss.consume_fire; $bspray += 1 end
...
def snap(label)
  ...
  printf("%03d %s st=%s score=%4d lives=%d n=%d e0=%s e1=%s boss=%s bhp=%2d pl=%s fire[e=%d s=%d d=%d y=%d] waves=%d\n",
         $step, label.ljust(30), $g.get_state.ljust(10),
         $g.get_score, $g.get_lives, $g.enemy_count,
         e0.ljust(8), e1.ljust(8), $g.boss.get_state.ljust(11), $g.boss.get_hp,
         $g.player.get_state.ljust(12), $ef, $bsingle, $bspread, $bspray, $waves_built)
  $step += 1
end

def run_n(n, label)
  pump(n); snap("pump x#{n} (#{label})")
end
...
$g2 = Shooter._create
$g2.start
10.times { 129.times { $g2.tick(DT) }; $g2.consume_wave if $g2.should_spawn_wave }
65.times { $g2.tick(DT) }
puts "Q00 quirk: state=#{$g2.get_state} boss=#{$g2.boss.get_state}"
$g2.boss_hit(90)
puts "Q01 quirk: one 90-dmg hit in P1 -> boss=#{$g2.boss.get_state} hp=#{$g2.boss.get_hp} (phase two, not dying)"
$g2.boss_hit(1)
puts "Q02 quirk: 1-dmg hit in P2 at 0hp -> boss=#{$g2.boss.get_state} hp=#{$g2.boss.get_hp} (phase three)"
$g2.boss_hit(1)
puts "Q03 quirk: 1-dmg hit in P3 -> boss=#{$g2.boss.get_state} (finally dying)"
