...
int step = 0;
int wavesBuilt = 0;
int ef = 0, bsingle = 0, bspread = 0, bspray = 0;
late Shooter g;
String pad(String s, int w) => s.length >= w ? s : s.padRight(w);
void buildWave() {
  wavesBuilt++;
  if (wavesBuilt == 1) {
    g.add_enemy(Enemy.create(0, 2, 0.0, 100));
    g.add_enemy(Enemy.create(1, 3, 0.75, 150));
  } else {
    g.add_enemy(Enemy.create(wavesBuilt % 3, 1, 0.0, 10));
    g.add_enemy(Enemy.create((wavesBuilt + 1) % 3, 1, 0.0, 10));
  }
}
void pump(int n) {
  for (var i = 0; i < n; i++) {
    ...
    for (var e = 0; e < g.enemy_count(); e++) {
      final en = g.enemies[e];
...
void snap(String label) {
  final e0 = g.enemy_count() > 0 ? g.enemies[0].get_state() : '-';
  final e1 = g.enemy_count() > 1 ? g.enemies[1].get_state() : '-';
  final st3 = step.toString().padLeft(3, '0');
  final sc = g.get_score().toString().padLeft(4);
  final bh = g.boss.get_hp().toString().padLeft(2);
...
void main() {
  ...
  final g2 = Shooter.create();
  g2.start();
  for (var w = 0; w < 10; w++) { for (var i = 0; i < 129; i++) { g2.tick(dt); } if (g2.should_spawn_wave()) g2.consume_wave(); }
  for (var i = 0; i < 65; i++) { g2.tick(dt); }
