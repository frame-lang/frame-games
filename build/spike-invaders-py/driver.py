from invaders import Invaders

DT = 1.0 / 64.0
step = 0
g = None

def ivus():
    return round(g.fleet.get_step_interval() * 1e6)
def pad(s, w):
    s = str(s)
    return s + " " * (w - len(s)) if len(s) < w else s
def lpad(s, w):
    s = str(s)
    return " " * (w - len(s)) + s if len(s) < w else s
def snap(label):
    global step
    fl = g.fleet
    print(f"{step:03d} {pad(label, 34)} "
          f"st={pad(g.get_state(), 13)} sc={lpad(g.get_score(), 4)} wv={g.get_wave()} lv={g.get_lives()} | "
          f"fl={pad(fl.get_state(), 9)} dir={lpad(fl.get_direction(), 2)} al={lpad(fl.alive_count(), 2)}/{lpad(fl.total(), 2)} "
          f"iv={lpad(ivus(), 6)} lr={lpad(fl.lowest_row(), 2)} | "
          f"pl={pad(g.player.get_state(), 12)} pz={1 if g.is_paused() else 0}")
    step += 1
def run(n, label):
    for _ in range(n):
        g.tick(DT)
    snap(f"pump x{n} ({label})")

g = Invaders._create()
snap("created")
g.start()
snap("start -> playing (fleet 55, iv=600000)")
print(f"OP  get_current_state_name={g.get_current_state_name()}")

run(39, "0.61s: fleet wants_to_step")
print(f"SIG consume_step={'true' if g.fleet.consume_step() else 'false'} (timer was >= interval)")

g.player_killed_invader(0)
snap("kill idx0 (+10, pace up)")
g.player_killed_invader(1)
g.player_killed_invader(2)
snap("kill idx1,2 (+20 more)")
g.player_killed_invader(1)
g.player_killed_invader(999)
g.player_killed_invader(-1)
snap("kill dead/oob idx: NO score change")

g.fleet_reached_edge()
snap("fleet_reached_edge -> stepping, dir flip")
run(1, "one tick: stepping -> marching")

g.pause()
snap("pause during PLAYING (push)")
run(64, "1.0s paused: fleet+player frozen")
g.resume()
snap("resume (pop -> playing)")

for i in range(3, 55):
    g.player_killed_invader(i)
snap("cleared fleet -> wave_complete")

g.pause()
snap("pause during WAVE_COMPLETE (push)")
run(64, "1.0s paused: wave timer frozen")
g.resume()
snap("resume (pop -> wave_complete)")

run(129, "2.0s: wave 2 begins, fleet reset")

g.player_hit()
snap("player_hit -> player_dying")
g.player_hit()
snap("player_hit while exploding: NO-OP")

g.pause()
snap("pause during PLAYER_DYING (push)")
run(64, "1.0s paused: explosion timer frozen")
g.resume()
snap("resume (pop -> player_dying)")

run(77, "1.2s: lives-1, invuln, -> playing")
run(96, "1.5s: invuln over -> alive")

g.fleet_reached_bottom()
snap("fleet_reached_bottom -> game_over")
g.restart()
snap("restart -> attract (reset)")

g2 = Invaders._create()
g2.start()
for life in range(3):
    g2.player_hit()
    for _ in range(180):
        g2.tick(DT)
print(f"DEATH after 3 hits: st={g2.get_state()} lives={g2.get_lives()} player={g2.player.get_state()}")

g3 = Invaders._create()
g3.start()
d0 = g3.fleet.get_direction()
g3.fleet_reached_edge(); g3.tick(DT)
d1 = g3.fleet.get_direction()
g3.fleet_reached_edge(); g3.tick(DT)
d2 = g3.fleet.get_direction()
print(f"DIR bounces: start={d0} after1={d1} after2={d2}")
